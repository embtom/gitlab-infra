#!/usr/bin/env python3
"""Interactively configure local GitLab settings and secrets."""
...
def read_setting(label: str) -> str:
...
def main() -> int:
    ...
    if (CONFIG_FILE.exists() or VAULT_FILE.exists()) and not confirm(
        "Replace existing local GitLab configuration and secrets vault?"
    ):
        print("Aborted.")
        return 0

    ldap_enabled = confirm("Enable GitLab LDAP authentication?")
    try:
        secrets = {}
        configuration: dict[str, object] = {"gitlab_service_ldap_enabled": ldap_enabled}
        if ldap_enabled:
            configuration["gitlab_service_ldap_host"] = read_setting(
                "LDAP certificate hostname"
            )
            if confirm("Is OpenLDAP running on the same host as GitLab?"):
                configuration["gitlab_service_ldap_host_address"] = "host-gateway"
            secrets["gitlab_service_ldap_bind_password"] = read_secret(
                "GitLab LDAP bind password"
            )
    ...
    print(f"Created local GitLab configuration: {CONFIG_FILE}")
    print(f"Created encrypted GitLab secrets vault: {VAULT_FILE}")
    return 0
